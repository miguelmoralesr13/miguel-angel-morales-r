...
const Projects: React.FC = () => {
  ...
  const fetchReadme = async (project: GitHubProject) => {
    ...
    try {
      ...
      // Decode base64 content and handle UTF-8 properly
            ...
            {error && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-sm mt-2">
                  ...
                  <a 
                    href="https://github.com/miguelmoralesr13" 
                    target="_blank" 
            ...
            {!loading && !error && (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {githubProjects.map((project) => (
                  <div key={project.id} className="bg-gray-900 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <div className="flex items-start justify-between mb-3">
                      <h4 className="text-lg font-bold text-white truncate">{project.name}</h4>
                      <span className="text-xs text-gray-400 ml-2">{formatDate(project.created_at)}</span>
                    </div>
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="flex-1 bg-green-600 hover:bg-green-700 text-white text-center py-2 px-4 rounded transition-colors duration-300 text-sm"
                      >
                        {t('projects.view_readme')}
                      </button>
                      <a 
                        href={project.html_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 bg-gray-700 hover:bg-gray-600 text-white text-center py-2 px-4 rounded transition-colors duration-300 text-sm"
                      >
                        {t('projects.github')}
                      </a>
                    </div>
          ...
          <div className="bg-gray-900 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-700">
              <h3 className="text-xl font-bold text-white">
                {t('projects.readme_modal.title')} {selectedProject?.name}
              </h3>
              <button
            ...
            <div className="flex justify-end p-6 border-t border-gray-700">
              <button
                onClick={() => setShowReadme(false)}
                className="bg-gray-700 hover:bg-gray-600 text-white px-6 py-2 rounded transition-colors duration-300"
              >
                {t('projects.readme_modal.close')}
              </button>
            </div>
          </div>
        </div>
